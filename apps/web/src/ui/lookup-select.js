// apps/web/src/ui/lookup-select.js

/**
 * Lookup 下拉选择（支持搜索 + 单/多选）
 *
 * ✅ UI（按你给的那套稳定实现）
 * - 默认收起：标题 + 已选摘要 + 下拉箭头（▾）+ 清空（×）+ 刷新（⟳）
 * - 点击箭头：弹出 overlay modal
 * - 在弹窗里搜索/选择 → 点“确定”提交；点“取消/关闭/点遮罩/Esc”丢弃草稿
 * - “×”一键清空
 *
 * 与你旧项目差异：
 * - 不依赖 supabase client
 * - 通过 opts.fetchLookup() 拉取候选数据（你这里将走 /api/admin/dropdowns/...）
 */
 ...
 * @param {function(): Promise<Array<{id:any,name:string,slug?:string}>>} opts.fetchLookup
 *   - 返回候选数组（注意：这里返回的是数组，不是 {items:[]})
 * @param {function(item): string} [opts.searchText]
 *   - 搜索用的文本（默认 name + slug）
 * @param {function(item): string} [opts.itemText]
 *   - 列表展示用文本（默认 name）
 */
export function createLookupSelect(opts) {
  ...
  /** @type {{id:any, name:string, slug?:string}[]} */
  let all = [];
  ...
  async function refresh({ useCache = true } = {}) {
    ...
    try {
      const cacheKey = `${title}|${mode}`;
      if (useCache && _cache.has(cacheKey)) {
        all = _cache.get(cacheKey);
      } else {
        all = await fetchLookup();
  ...
  function openModal() {
    ...
    function renderChips() {
      ...
      for (const item of draft.values()) {
        ...
        chips.appendChild(el('span', { class: 'af-chip' }, [document.createTextNode(itemText(item)), x]));
      }
    }

    function renderList() {
      const q = norm(input.value);
      const shown = q ? all.filter(r => contains(searchText(r), q)) : all;
  ...
  // compact view events
  btnArrow.addEventListener('click', () => openModal());
